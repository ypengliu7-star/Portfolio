...
export function TrialTailsCase({ project, next }: { project: Project; next: Project }) {
  return <main className="case-study tails-native">
    ...
    <section className="tails-detail-library">
      <header><p className="tails-label">06 / HIGH-RES DESIGN BOARDS</p><h2>从研究到原型，<br />让设计过程按章节展开。</h2><p>所有画面保持原始比例，不再机械切割。点击任意画板可查看高清原图。</p></header>
      <div className="tails-board-grid">
        {detailBoards.map(([file,label],index)=><article key={file} className={index % 3 === 0 ? "is-featured" : ""}>
          <div><span>{String(index+1).padStart(2,"0")}</span><h3>{label}</h3><a href={detailImage(file)} target="_blank" rel="noreferrer">高清原图 ↗</a></div>
          <a className="tails-board-image" href={detailImage(file)} target="_blank" rel="noreferrer"><img src={detailImage(file)} alt={`TRIAL TAILS ${label}`} loading="lazy" /></a>
        </article>)}
      </div>
    </section>
    ...
    <style>{`
      ...
      .tails-detail-library{padding:130px 7vw;background:#241713;color:#f4f0e7}.tails-detail-library>header{display:grid;grid-template-columns:180px 1fr .55fr;gap:4vw;align-items:end;margin-bottom:80px}.tails-detail-library>header>p:last-child{margin:0;font-size:14px;line-height:1.8;color:#b9aaa2}.tails-detail-library h2{margin:0;font-size:clamp(42px,5.5vw,82px);line-height:1.04;letter-spacing:-.055em;font-weight:500}.tails-board-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:70px 18px}.tails-board-grid article{min-width:0;border-top:1px solid rgba(255,255,255,.18);padding-top:18px}.tails-board-grid article.is-featured{grid-column:1/-1}.tails-board-grid article>div{display:grid;grid-template-columns:42px 1fr auto;gap:14px;align-items:center;margin-bottom:16px}.tails-board-grid article>div span{font-size:22px;color:var(--orange)}.tails-board-grid article h3{margin:0;font-size:17px;font-weight:500}.tails-board-grid article>div a{font-size:9px;letter-spacing:.12em;color:#ef9a78}.tails-board-image{display:block;overflow:hidden;background:#f5f1e8}.tails-board-image img{display:block;width:100%;height:auto;transition:transform .45s}.tails-board-image:hover img{transform:scale(1.006)}
      @media(max-width:800px){
        ...
        .tails-detail-library>header{grid-template-columns:1fr;gap:25px}.tails-board-grid{grid-template-columns:1fr;gap:55px}.tails-board-grid article.is-featured{grid-column:auto}.tails-board-grid article>div{grid-template-columns:36px 1fr}.tails-board-grid article>div a{grid-column:2}.tails-archive details>div{grid-template-columns:repeat(2,1fr)}.tails-archive a{aspect-ratio:4/3}.tails-overview h2,.tails-problem h2,.tails-system h2,.tails-journey h2,.tails-interface h2,.tails-result h2,.tails-archive h2{font-size:40px}
      }
    `}</style>
  </main>;
}
