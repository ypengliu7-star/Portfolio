...
export function LondonUnderworldCase({ project, next }: { project: Project; next: Project }) {
  return <main className="case-study underworld">
    ...
    <section className="uw-gallery">
      ...
      <div className="uw-photo-wall">{fieldPhotos.filter(file=>file.endsWith(".jpg")).map((file,index)=><a href={uploadedImage(file)} target="_blank" rel="noreferrer" key={file} className={index % 7 === 0 ? "is-wide" : ""}><img src={uploadedImage(file)} alt={`伦敦地下局真实现场照片 ${String(index+1).padStart(2,"0")}`} loading="lazy" /><span>LIVE / {String(index+1).padStart(2,"0")}</span></a>)}</div>
      <div className="uw-private-feedback">
        <div><span>PLAYER FEEDBACK</span><h3>玩家反馈记录</h3><p>为保护参与者隐私，朋友圈昵称、头像与文字内容已统一模糊处理，不提供原图放大。</p></div>
        <div>{fieldPhotos.filter(file=>file.endsWith(".png")).map((file,index)=><figure key={file}><span><img src={uploadedImage(file)} alt={`隐私保护后的玩家反馈记录 ${index+1}`} loading="lazy" /></span><figcaption>FEEDBACK / {String(index+1).padStart(2,"0")}</figcaption></figure>)}</div>
      </div>
    </section>
    ...
    <style>{`
      ...
      .uw-private-feedback{display:grid;grid-template-columns:.55fr 1.45fr;gap:6vw;align-items:end;margin-top:80px;padding-top:28px;border-top:1px solid rgba(215,208,195,.18)}.uw-private-feedback>div:first-child>span{font-size:8px;letter-spacing:.18em;color:#b78b4e}.uw-private-feedback h3{margin:14px 0 12px;font-family:Georgia,"Songti SC",serif;font-size:29px;font-weight:400}.uw-private-feedback p{max-width:420px;margin:0;color:#96877d;font-size:12px;line-height:1.8}.uw-private-feedback>div:last-child{display:flex;justify-content:flex-end;align-items:flex-end;gap:12px;flex-wrap:wrap}.uw-private-feedback figure{width:86px;margin:0}.uw-private-feedback figure>span{position:relative;display:block;height:112px;overflow:hidden;background:#271819}.uw-private-feedback figure>span:after{content:"PRIVACY";position:absolute;inset:0;display:grid;place-items:center;background:repeating-linear-gradient(135deg,rgba(27,14,15,.35) 0 8px,rgba(27,14,15,.62) 8px 16px);color:rgba(255,255,255,.72);font-size:7px;letter-spacing:.12em}.uw-private-feedback img{display:block;width:100%;height:100%;object-fit:cover;filter:blur(8px) saturate(.45) brightness(.68);transform:scale(1.18)}.uw-private-feedback figcaption{margin-top:7px;font-size:7px;letter-spacing:.1em;color:#766862}
